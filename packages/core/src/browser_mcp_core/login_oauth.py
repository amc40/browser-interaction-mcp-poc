"""GitHub sign-in gate for an app's out-of-band login page.
...
the site's login page: a person visiting that URL is bounced through GitHub,
and only the one configured ``github_user_id`` is let in.
...
def callback_path(login_path: str) -> str:
    """Return the GitHub redirect URI's path for a login page at ``login_path``.

    A sub-path rather than a sibling so that one registered redirect URI with
    wildcard matching covers both an app's callbacks - see
    docs/scaling-plan.md D10.

    Args:
        login_path: Where the login page is served.

    Returns:
        The callback path to register the route at.
    """
    return f"{login_path}/auth/callback"
...
class ScopedSessionMiddleware(SessionMiddleware):
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        path = scope.get("path", "")
        # A path-segment match, not a bare prefix: `/<login-path>-elsewhere` is
        # a different route and has no business being handed a session.
...
def login_session_middleware(settings: CoreSettings, login_path: str) -> Middleware:
    ...
    Args:
        ...
        login_path: Path prefix the session is scoped to.
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            login_path: Where the login page is served, which is what a
                successful sign-in redirects back to.
        ...
        self._redirect_uri = f"{base_url}{callback_path(login_path)}"
