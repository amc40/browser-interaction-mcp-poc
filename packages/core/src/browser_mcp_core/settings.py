"""Runtime configuration every app shares, read from the environment or ``.env``."""
...
class CoreSettings(BaseSettings):
    """The settings every app has, whatever site it automates.

    A site subclasses this to add its own - see
    ``browser_mcp_sainsburys.settings.SainsburysSettings``. Subclassing rather
    than composing is what keeps :func:`browser_mcp_core.redaction.build_redactor`
    complete: it walks ``model_fields`` for ``SecretStr``, so a site's own
    credential is covered by the same convention and there is no second place
    to remember.

    Every field can be overridden with a ``BROWSER_MCP_``-prefixed environment
    variable, for example ``BROWSER_MCP_RATE_LIMIT_PER_SECOND=0.5``. The prefix
    is shared by every app for now; docs/scaling-plan.md D2 makes it per-app in
    stage 3, when each app gets an environment file of its own that two apps
    could otherwise collide in.
    """
