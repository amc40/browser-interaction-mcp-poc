...
from browser_mcp_core.middleware import SecretRedactionMiddleware
from browser_mcp_core.redaction import (
    MIN_SECRET_LENGTH,
...
def test_build_redactor_tolerates_settings_with_no_secrets() -> None:
    assert build_redactor(FakeSettings()).redact(SECRET) == SECRET
...
async def test_tool_errors_reach_the_client_redacted(
    ...
    server = build_server(FAKE_SITE, settings)
