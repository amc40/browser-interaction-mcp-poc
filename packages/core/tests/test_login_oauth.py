...
from browser_mcp_core.login_oauth import (
    BrowserGithubAuth,
    callback_path,
...
# A path of this test's own, not any real site's: the whole point of the
# parameter is that core no longer has one of its own.
LOGIN_PATH = "/fake-login"
_CALLBACK_PATH = callback_path(LOGIN_PATH)
...
def _app(transport: httpx.MockTransport) -> Starlette:
    ...
    settings = _settings()
    auth = BrowserGithubAuth(settings, LOGIN_PATH, transport=transport)
    ...
    return Starlette(
        routes=[
            ...
            Route(_CALLBACK_PATH, callback),
            Route(_BYPASS_PATH, bypassed),
        ],
        middleware=[login_session_middleware(settings, LOGIN_PATH)],
    )
...
async def test_an_unauthenticated_visitor_is_bounced_to_github(
    ...
    assert query["redirect_uri"] == [f"{_BASE_URL}{_CALLBACK_PATH}"]
...
async def test_the_allowed_account_completes_the_round_trip(
    ...
    response = await client.get(f"{_CALLBACK_PATH}?code=abc&state={state}")
...
async def test_a_state_issued_to_another_browser_is_refused() -> None:
    ...
    async with _client(_github()) as attacker, _client(_github()) as victim:
        ...
        response = await victim.get(f"{_CALLBACK_PATH}?code=abc&state={stolen}")
...
async def test_a_callback_with_no_session_at_all_is_refused(
    ...
    response = await client.get(f"{_CALLBACK_PATH}?code=abc&state=never-issued")
...
def test_missing_oauth_credentials_is_a_configuration_error() -> None:
    with pytest.raises(ValueError, match="OAuth app credentials"):
        BrowserGithubAuth(CoreSettings(transport="stdio"), LOGIN_PATH)


def test_a_session_without_oauth_credentials_is_a_configuration_error() -> None:
    with pytest.raises(ValueError, match="OAuth app credentials"):
        login_session_middleware(CoreSettings(transport="stdio"), LOGIN_PATH)
