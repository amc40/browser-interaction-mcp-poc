...
from browser_mcp_core.auth import build_auth_provider
...
async def _call_server_info(settings: FakeSettings | None = None) -> None:
    """Call core's own tool, letting any error propagate."""
    async with Client(build_server(FAKE_SITE, settings or FakeSettings())) as client:
...
async def test_the_allowed_account_is_configurable(
    ...
    await _call_server_info(FakeSettings(github_user_id="12345"))
...
async def test_authorisation_precedes_rate_limiting(
    ...
    """
    server = build_server(FAKE_SITE, FakeSettings(rate_limit_burst=1))
...
def _http_settings(**overrides: object) -> FakeSettings:
    return FakeSettings(
        transport="http",
        github_client_id="Ov23liExample",
        github_client_secret=SecretStr("not-a-real-secret"),
...
def test_stdio_transport_has_no_auth_provider() -> None:
    ...
    assert build_auth_provider(FakeSettings()) is None
...
def test_stdio_is_left_unauthenticated_loudly(
    ...
    with caplog.at_level("WARNING"):
        build_auth_provider(FakeSettings())
