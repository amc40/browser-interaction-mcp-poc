"""Tests for the out-of-band login coordinator, against a fake site.
...
from browser_mcp_core import login_flow
...
# A fake site's, so that nothing here depends on a real one.
_SLUG = "fake"
_DISPLAY_NAME = "Fake Grocer"
_WORKER_MODULE = "browser_mcp_core_fake_worker"
...
def flow_factory() -> Iterator[object]:
    ...
    def make(storage_state_path: Path) -> tuple[LoginFlow, list[_FakeWorker]]:
...
def test_worker_success_is_surfaced_as_done(
    ...
    workers[0].report("done", f"Your {_DISPLAY_NAME} session has been refreshed.")
    workers[0].exit(0)
...
def test_finished_attempts_are_forgotten_after_the_retention_window(
    ...
    monkeypatch.setattr(login_flow, "TERMINAL_RETENTION_SECONDS", -1.0)
    flow, workers = flow_factory(tmp_path / "session.json")  # type: ignore[operator]
    flow.start("hunter2")
