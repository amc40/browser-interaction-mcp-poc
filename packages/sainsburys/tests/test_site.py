...
# ---------------------------------------------------------------------------
# The login steps, driven by core's refresh_session
# ---------------------------------------------------------------------------
...
def test_refresh_session_fills_and_submits_the_login_form(
    ...
    login_steps.refresh_session(
        sainsburys.SainsburysLoginSteps(),
        "alan@example.com",
        "hunter2",
...
def test_refresh_session_raises_when_mfa_is_required_and_no_otp_is_given(
    ...
    with pytest.raises(NotLoggedInError, match="verification code"):
        login_steps.refresh_session(
            sainsburys.SainsburysLoginSteps(),
            "alan@example.com",
            "hunter2",
...
def test_refresh_session_raises_when_still_not_logged_in_afterwards(
    ...
    with pytest.raises(NotLoggedInError, match="check the password"):
        login_steps.refresh_session(
            sainsburys.SainsburysLoginSteps(),
            "alan@example.com",
            "wrong-password",
